...
!function (a) {
  ...
  var c = function (c, d) {
    ...
    B = function () {
      ...
      f || (b.append(a('<td>').addClass('separator')), c.append(a('<td>').append(a('<button>').addClass('btn btn-primary').attr('data-action', 'togglePeriod'))), e.append(a('<td>').addClass('separator'))),
      a('<div>').addClass('timepicker-picker').append(a('<table>').addClass('table-condensed').append([b,
      c,
      e]))
    },
    C = function () {
    ...
    D = function () {
      ...
      return d.showTodayButton && b.push(a('<td>').append(a('<a>').attr('data-action', 'today').append(a('<span>').addClass(d.icons.today)))),
      !d.sideBySide && z() && y() && b.push(a('<td>').append(a('<a>').attr('data-action', 'togglePicker').append(a('<span>').addClass(d.icons.time)))),
      d.showClear && b.push(a('<td>').append(a('<a>').attr('data-action', 'clear').append(a('<span>').addClass(d.icons.clear)))),
      d.showClose && b.push(a('<td>').append(a('<a>').attr('data-action', 'close').append(a('<span>').addClass(d.icons.close)))),
    ...
    N = function () {
      for (var b = [
      ], c = l.clone().startOf('y').hour(12); c.isSame(l, 'y'); ) b.push(a('<span>').attr('data-action', 'selectMonth').addClass('month').text(c.format('MMM'))),
      c.add(1, 'M');
    ...
    P = function () {
      ...
      e = l.clone().add(6, 'y'),
      f = '';
      for (a.find('.disabled').removeClass('disabled'), d.minDate && d.minDate.isAfter(c, 'y') && b.eq(0).addClass('disabled'), b.eq(1).text(c.year() + '-' + e.year()), d.maxDate && d.maxDate.isBefore(e, 'y') && b.eq(2).addClass('disabled'); !c.isAfter(e, 'y'); ) f += '<span data-action="selectYear" class="year' + (c.isSame(k, 'y') ? ' active' : '') + (M(c, 'y') ? '' : ' disabled') + '">' + c.year() + '</span>',
      c.add(1, 'y');
      a.find('td').html(f)
    },
    Q = function () {
      ...
      if (z()) {
        ...
        e.append('<td data-action="selectDay" class="day' + f + '">' + c.date() + '</td>'),
        c.add(1, 'd');
        g.find('tbody').empty().append(i),
        O(),
        P()
      }
    },
    R = function () {
      ...
      e.append('<td data-action="selectHour" class="hour' + (M(c, 'h') ? '' : ' disabled') + '">' + c.format(f ? 'HH' : 'hh') + '</td>'),
      c.add(1, 'h');
      b.empty().append(d)
    },
    S = function () {
      for (var b = o.find('.timepicker-minutes table'), c = l.clone().startOf('h'), e = [
      ], f = a('<tr>'), g = 1 === d.stepping ? 5 : d.stepping; l.isSame(c, 'h'); ) c.minute() % (4 * g) === 0 && (f = a('<tr>'), e.push(f)),
      f.append('<td data-action="selectMinute" class="minute' + (M(c, 'm') ? '' : ' disabled') + '">' + c.format('mm') + '</td>'),
      c.add(g, 'm');
      b.empty().append(e)
    },
    T = function () {
      for (var b = o.find('.timepicker-seconds table'), c = l.clone().startOf('m'), d = [
      ], e = a('<tr>'); l.isSame(c, 'm'); ) c.second() % 20 === 0 && (e = a('<tr>'), d.push(e)),
      e.append('<td data-action="selectSecond" class="second' + (M(c, 's') ? '' : ' disabled') + '">' + c.format('ss') + '</td>'),
      c.add(5, 's');
      b.empty().append(d)
    },
    U = function () {
      var a = o.find('.timepicker span[data-time-component]');
      f || o.find('.timepicker [data-action=togglePeriod]').text(k.format('A')),
      a.filter('[data-time-component=hours]').text(k.format(f ? 'HH' : 'hh')),
    ...
    Z = {
      next: function () {
        l.add(q[i].navStep, q[i].navFnc),
        Q()
      },
      previous: function () {
        l.subtract(q[i].navStep, q[i].navFnc),
        Q()
      },
      ...
      selectMonth: function (b) {
        ...
        l.month(c),
        i === p ? (W(k.clone().year(l.year()).month(l.month())), d.inline || X())  : (I( - 1), Q())
      },
      selectYear: function (b) {
        var c = parseInt(a(b.target).text(), 10) || 0;
        l.year(c),
        i === p ? (W(k.clone().year(l.year())), d.inline || X())  : (I( - 1), Q())
      },
    ...
    if (j.destroy = function () {
      ...
      d.defaultDate && '' === e.val().trim() && void 0 === e.attr('placeholder') && W(d.defaultDate),
      j
    }, j.locale = function (a) {
      ...
      if ('[object Object]' !== {
      }.toString.call(b)) throw new TypeError('widgetPositioning() expects an object variable');
      if (b.horizontal) {
    ...
    e.is('input') && 0 !== e.val().trim().length ? W(bb(e.val().trim()))  : d.defaultDate && void 0 === e.attr('placeholder') && W(d.defaultDate),
    d.inline && _(),
    j
  };
